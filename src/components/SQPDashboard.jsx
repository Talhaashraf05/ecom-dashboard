...
export default function SQPDashboard({ csvTexts = [], onAddFiles, onBack, projectName }) {
  ...
  // Parse all saved CSV texts on mount / when csvTexts changes
  useEffect(() => {
    const parsed = csvTexts.map(f => parseCSV(f.text)).filter(Boolean);
    const sorted = parsed.sort((a, b) => a.sortKey - b.sortKey).slice(-12);
    setAllWeeks(sorted);
    setSelectedWeekIdx(sorted.length - 1);
  }, [csvTexts]);

  const handleFiles = useCallback(async files => {
    // read File objects to text, then delegate to parent for persistence
    const results = [];
    for (const f of files) results.push({ name: f.name, text: await f.text() });
    if (onAddFiles) onAddFiles(results);
  }, [onAddFiles]);
